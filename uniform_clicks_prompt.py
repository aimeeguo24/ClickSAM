...
def compute_energy(points, region):
    ...
    ycoords, xcoords = np.meshgrid(np.arange(h), np.arange(w))
...
def test_gamma():
    for i in range(0,647):
        # img=Image.open("data/BUSIresized/BUSI2Dgts/"+str(i)+".png")
        n_points=1
        if i < 437:
            img = iio.imread("data/BUSIresized/BUSI2Dgts/"+str(i)+".png")
            kernel = np.ones((5, 5), np.uint8)
            img_erosion = cv2.erode(img, kernel, iterations=2)
            binary_mask = img_erosion > 254
        ...
        sp = SpreadPoints(
            ...
            image_index=i,
            segment_index=i,
            segment_type="false_negatives",
        )
        sp.set_schedule({"tmax": 2000.0, "tmin": 0.1, "steps": 100, "updates": 0})
        ...
        with open(
            ...
            pickle.dump(best_points, points_file)
...
if __name__ == "__main__":
    ...
    step_multiplier = 1 #originally 10
    step_size = n_cpus * step_multiplier
    #mp_results_step=test_gamma()
    test_gamma()
    ...
    #         )
    #     )
    #     mp_results_step = click_generator(baseline_dir,list(range(0,779)),segment_files,anneal_schedule,iou_max,max_clicks_per_segment,max_clicks_per_kind)
